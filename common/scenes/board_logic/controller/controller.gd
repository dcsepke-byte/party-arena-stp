...
@export var STAR_COST := 20
@export var MAX_TURNS := 10

# Party Arena: Stern-Mechanik (star-economy.md, Sektion 3.2.7)
# Trackt Stern-Käufe pro Spieler und Runde
var _stars_purchased_this_round: Dictionary = {}
...
#			Global.save_storage()
	...
	if server:
		if lobby.star_space.is_empty():
			await relocate_star()
		else:
			lobby.broadcast(show_star_space.bind(lobby.star_space))
...
func show_tutorial():
	...
	await announce("CONTEXT_TUTORIAL_STARS")
...
@rpc func star_collected():
	var old_node: NodeBoard = get_active_star_space()
	await old_node.play_star_collection_animation()
	old_node.star_active = false
	acknowledge()

@rpc func show_star_space(path: NodePath):
	var new_node: NodeBoard = get_node(path)
	if not new_node is NodeBoard or not lobby.is_ancestor_of(new_node):
		# Server is misbehaving
		lobby.leave()
		return
	new_node.star_active = true

@rpc func star_relocated(path: NodePath):
	var new_node: NodeBoard = get_node(path)
	if not new_node is NodeBoard or not lobby.is_ancestor_of(new_node):
		# Server is misbehaving
		lobby.leave()
		return
	var old_focus: Node3D = camera_focus
	camera_focus = new_node
	new_node.star_active = true
	await _camera_focus_aquired
	await announce("CONTEXT_STAR_PLACED")
	camera_focus = old_focus
	await _camera_focus_aquired
	lobby.star_space = path
	acknowledge()

# Party Arena: Get all star shop fields on the board (star-economy.md 3.1)
func _get_star_shop_fields() -> Array[NodeBoard]:
	var fields: Array[NodeBoard] = []
	var all_nodes: Array = Utility.get_nodes_in_group(lobby, "nodes")
	for node in all_nodes:
		if node is NodeBoard and node.type == NodeBoard.FELD_TYP.STERN_SHOP:
			fields.append(node as NodeBoard)
	return fields

func relocate_star() -> void:
	var star_fields: Array[NodeBoard] = _get_star_shop_fields()
	if star_fields.size() == 0:
		push_warning("relocate_star: No STERN_SHOP fields on board")
		return

	var old_node: NodeBoard = get_active_star_space()

	# Deactivate old statue
	if old_node:
		wait_for_acknowledgement()
		lobby.broadcast(star_collected)
		await players_acknowledged
		old_node.star_active = false

	# Build candidate list: all star fields except current (star-economy.md 3.3.2)
	var candidates: Array[NodeBoard] = []
	for field in star_fields:
		if field != old_node:
			candidates.append(field)

	if candidates.size() == 0:
		# Only one star shop field on board; statue stays
		if old_node:
			old_node.star_active = true
		return

	# Separate occupied vs unoccupied (star-economy.md 3.3.3)
	var unoccupied: Array[NodeBoard] = []
	var occupied: Array[NodeBoard] = []
	for field in candidates:
		...
		if has_player:
			...
			unoccupied.append(field)

	# Choose: prefer unoccupied, fallback to any candidate (star-economy.md 3.3.3)
	var new_node: NodeBoard
	if unoccupied.size() > 0:
		new_node = unoccupied[randi() % unoccupied.size()]
	else:
		new_node = candidates[randi() % candidates.size()]

	# Activate new statue
	lobby.star_space = get_path_to(new_node)
	new_node.star_active = true

	wait_for_acknowledgement()
	lobby.broadcast(star_relocated.bind(lobby.star_space))
	await players_acknowledged
...
func _on_next_player():
	# Party Arena: Reset star purchase tracking at start of each round
	if player_turn == 1:
		_stars_purchased_this_round.clear()
...
func prepare_minigame():
	...
	for p in players:
		if randi() % 2 == 0:
			team_a.push_back(p.info.player_id)
		else:
			team_b.push_back(p.info.player_id)
	...
	state.minigame_teams = [team_a, team_b]

	match [team_a.size(), team_b.size()]:
		[4, 0]:
...
func _step(player: PlayerBoard, previous_space: NodeBoard, last: bool) -> Array:
	...
	if player.space.type == NodeBoard.FELD_TYP.ITEM_SHOP or player.space.type == NodeBoard.FELD_TYP.STERN_SHOP:
		if not stopped:
	...
	# Or we're right before multiple pathways, a star shop or a shop
	...
	var next_step_blocking = not last and (space.next.size() > 1 or
			space.type == NodeBoard.FELD_TYP.ITEM_SHOP or space.type == NodeBoard.FELD_TYP.STERN_SHOP)
...
func land_on_space(player: PlayerBoard):
	...
	# Apply field type effect based on Party Arena FELD_TYP system
	match player.space.type:
		NodeBoard.FELD_TYP.START:
			# Start (field-start.md) — Lap-Bonus
			player.cookies += 3
		NodeBoard.FELD_TYP.STERN_SHOP:
			# Sternen-Shop (star-economy.md) -- Stern-Kauf nur auf AKTIVEM Feld
			var active_star: NodeBoard = get_active_star_space()
			if player.space == active_star:
				if not player.info.is_ai():
					await buy_star(player)
				else:
					ai_purchase_star(player)
					await get_tree().create_timer(1).timeout
			# Sternen-Shop (field-star-shop.md) — Stern kaufen
			if not player.info.is_ai():
				start_timer_for_player(player.info.addr)
				$Screen/Shop.player_do_shopping(player)
				await $Screen/Shop.shopping_completed
				...
				$Screen/Shop.ai_do_shopping(player)
				await get_tree().create_timer(1).timeout
		NodeBoard.FELD_TYP.ITEM_SHOP:
			# Item-Shop (field-item-shop.md) — Item kaufen
			if not player.info.is_ai():
				start_timer_for_player(player.info.addr)
				$Screen/Shop.player_do_shopping(player)
				await $Screen/Shop.shopping_completed
				...
				$Screen/Shop.ai_do_shopping(player)
				await get_tree().create_timer(1).timeout
		NodeBoard.FELD_TYP.EREIGNIS:
			# Ereignis (field-event.md) — Zufalls-Ereignis (ersetzt altes NOLOK/GNU)
			if len(trigger_event.get_connections()) > 0:
				trigger_event.emit(player, player.space)
				await _event_completed
			else:
				# Fallback: einfacher Münz-Effekt −5 bis +5
				var amount: int = randi() % 11 - 5
				player.cookies = max(0, player.cookies + amount)
				push_warning("Player stepped on EREIGNIS space, but no board event"
					+ " handler is registered, using fallback coin effect.")
				await get_tree().create_timer(1).timeout
		NodeBoard.FELD_TYP.GLUECK_PECH:
			# Glück/Pech (field-luck.md) — Münz-Zufall
			if randi() % 2 == 0:
				player.cookies += 5
			else:
				player.cookies = max(0, player.cookies - 5)
		NodeBoard.FELD_TYP.MUENZ_BONUS:
			# Münz-Bonus (field-coin-bonus.md) — garantierter Bonus
			player.cookies += 5
		NodeBoard.FELD_TYP.MINISPIEL:
			# Mini-Spiel (field-minigame.md) — Minispiel-Kategorie-Markierung
			player.cookies += 3

	player_turn += 1
	next_player.emit()

# Moves a player num spaces forward.
...
func _get_player_placement(p: PlayerBoard) -> int:
	var placement := 1
	for p2 in players:
		if p2.stars > p.stars or p2.stars == p.stars and p2.cookies > p.cookies:
			placement += 1
...
# Gets the reference to the node on which the star statue currently stands
# collected
# Party Arena: Get the currently active star statue node
func get_active_star_space() -> NodeBoard:
	if lobby.star_space.is_empty():
		return null
	return get_node_or_null(lobby.star_space) as NodeBoard


# Party Arena: AI Stern-Kauf (star-economy.md)
func ai_purchase_star(player: PlayerBoard) -> void:
	var pid: int = player.info.player_id

	# Check max 1 purchase per player per round
	if _stars_purchased_this_round.get(pid, false):
		return

	# Verify player is on the active star statue
	var active_star: NodeBoard = get_active_star_space()
	if player.space != active_star:
		return

	if player.cookies >= STAR_COST:
		player.cookies -= STAR_COST
		player.stars += 1
		_stars_purchased_this_round[pid] = true

# Party Arena: Stern-Kauf (star-economy.md, Sektion 3.2)
# Kauf-Fenster A: Kauf bei Landung auf aktivem Sternen-Shop
func buy_star(player: PlayerBoard) -> void:
	var pid: int = player.info.player_id

	# Check max 1 purchase per player per round (star-economy.md 3.2.7)
	if _stars_purchased_this_round.get(pid, false):
		await announce("CONTEXT_STAR_ALREADY_BOUGHT")
		return

	# Verify player is on the active star statue (star-economy.md 3.2.3)
	var active_star: NodeBoard = get_active_star_space()
	if player.space != active_star:
		return

	if player.cookies >= STAR_COST:
		if await ask_yes_no("CONTEXT_STAR_WANT_BUY"):
			await get_tree().create_timer(0.5).timeout
			player.cookies -= STAR_COST
			player.stars += 1
			_stars_purchased_this_round[pid] = true
			await announce("CONTEXT_STAR_COLLECTED", {"player": player.name})
			# Statue wandert nach Kauf (star-economy.md 3.3)
			await relocate_star()
	else:
		await announce("CONTEXT_STAR_CANT_AFFORD")
...
@rpc func minigame_duel_reward_animation(reward: Lobby.MINIGAME_DUEL_REWARDS) -> void:
	...
	elif reward_name == "ONE_STAR":
		$Screen/DuelReward/Value.text = "CONTEXT_LABEL_STEAL_ONE_STAR"
	else:
