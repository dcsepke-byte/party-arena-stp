...
func discover_character(filename: String) -> void:
	...
	if preload("res://common/scripts/character.gd").instance_has(scene):
...
func load_character_splash(name: String) -> Texture:
	return load(CHARACTER_PATH + "/" + name + "/" + CHARACTER_SPLASHNAME) as Texture

func load_character_icon(name: String) -> Texture:
	return load(CHARACTER_PATH + "/" + name + "/" + CHARACTER_ICONNAME) as Texture
