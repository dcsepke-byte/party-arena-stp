...
class PlayerState:
	...
	## [b] Only use this in the network layer and not in plugin code! [/b]
	func encode():
		return [info.player_id, cookies, stars, items, roll_modifiers]
...
## Node on the board that currently hosts the active star statue
var star_space := NodePath()
