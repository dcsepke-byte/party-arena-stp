...
func _ready():
	...
	for player in lobby.playerstates:
		...
		$Summary/Stats/Stars/Entries.get_node("Player" + str(i)).text = str(player.stars)
	...
	var winner = []
	for player in lobby.playerstates:
		if not winner or (winner[0].stars < player.stars or (winner[0].stars == player.stars and winner[0].cookies < player.cookies)):
			winner = [player]
		elif winner and winner[0].stars == player.stars and winner[0].cookies == player.cookies:
