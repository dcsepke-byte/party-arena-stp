...
func _init():
	...
	settings["main/star_cost"] = Settings.new_range("MENU_SETTINGS_CAKE_COST", 30, 10, 65535)
...
func _on_setting_changed(setting: Settings):
	match setting.name:
		...
		"MENU_SETTINGS_CAKE_COST":
			overrides.star_cost = setting.get_value()
		"MENU_SETTINGS_TURNS":
				...
				if "Controller" in groups:
					for prop in range(scene.get_node_property_count(i)):
						match scene.get_node_property_name(i, prop):
							"STAR_COST":
								star_cost = int(scene.get_node_property_value(i, prop))
							"MAX_TURNS":
								max_turns = int(scene.get_node_property_value(i, prop))
		settings["main/star_cost"].update_value(star_cost)
...
func goto_minigame() -> void:
	...
	for i in r_players.size():
		...
		playerstates[i].stars = r_players[i].stars
	...
	match minigame_type:
		...
		MINIGAME_TYPES.DUEL:
			if len(placement) == 2:
				...
				match minigame_reward.duel_reward:
					...
					MINIGAME_DUEL_REWARDS.ONE_STAR:
						# Party Arena Anti-Rule (star-economy.md 3.6): Stars cannot be stolen
						# Changed to coin transfer instead of star theft
						var star_value_cookies: int = 15
						var steal_amount: int = int(min(losing_player.cookies, star_value_cookies))
						winning_player.cookies += steal_amount
						losing_player.cookies -= steal_amount
						minigame_summary.reward = steal_amount
			else:
		...
		MINIGAME_TYPES.NOLOK_SOLO:
			if not placement:
				...
				# Party Arena Anti-Rule (star-economy.md 3.6): Stars cannot be stolen
				# Changed to coin loss instead of star loss
				minigame_summary.reward = min(player.cookies, 10)
				player.cookies -= minigame_summary.reward
			else:
				minigame_summary.reward = 0
...
func load_board_state(controller: Node3D) -> void:
	controller.STAR_COST = overrides.star_cost
	controller.MAX_TURNS = overrides.max_turns

	if star_space:
		var star_node: Node3D = controller.get_node(star_space)
		star_node.star_active = true
	...
	for i in range(r_players.size()):
		...
		r_players[i].stars = playerstates[i].stars
...
@rpc func load_savegame(data: Dictionary) -> void:
	...
	for i in len(savegame.players):
		...
		playerstate.stars = savegame.players[i].stars
	...
	star_space = savegame.board_state.star_space
