...
# Party Arena: Zuckerwald-Board
# Schwierigkeit: ★★☆☆☆
# Game Bible: design/gdd/world-zuckerwald.md
...
const REFERENCE_LAYOUT: Array[int] = [
		...
		NodeBoard.FELD_TYP.STERN_SHOP,  # 22,
		NodeBoard.FELD_TYP.MINISPIEL,  # 23,
		NodeBoard.FELD_TYP.GLUECK_PECH,  # 24,
		...
		NodeBoard.FELD_TYP.EREIGNIS,  # 29,
		NodeBoard.FELD_TYP.ITEM_SHOP,  # 30,
		NodeBoard.FELD_TYP.MINISPIEL,  # 31,
		NodeBoard.FELD_TYP.MINISPIEL,  # 32,
		NodeBoard.FELD_TYP.MINISPIEL,  # 33,
		NodeBoard.FELD_TYP.STERN_SHOP,  # 34,
		NodeBoard.FELD_TYP.GLUECK_PECH,  # 35,
...
# Insel-Metadaten
var board_id: String = "zuckerwald"
var board_name: String = "Zuckerwald"
var difficulty: int = 2
var item_price_tier: int = 0

# Abzweigungs-Daten (Single Source of Truth: board.json)
# Einstieg/Ausstieg/Δ gemäß Game Bible world-zuckerwald.md
const BRANCHES: Array[Dictionary] = [
	{
		"name": "Schokoladen-Fluss",
		"entry": 16,
		"exit": 20,
		"delta": 1,
		"fields": [33, 34, 35, 36],
	...
	{
		"name": "Zuckerwatte-Wolken",
		"entry": 25,
		"exit": 32,
		"delta": -2,
		"fields": [37, 38, 39, 40],
		"rule": "zuckerwatte_bonus_minigame",
	},
]
...
func _apply_reference_layout() -> void:
	if not has_node("Nodes"):
		push_error("zuckerwald board: 'Nodes' container not found!")
		return
	...
	if node_boards.is_empty():
		push_error("zuckerwald board: No NodeBoard instances found!")
		return

	node_boards.sort_custom(func(a: Node, b: Node) -> bool: return a.name.naturalnocase_to(b.name) < 0)
	...
	print("zuckerwald board: Applied reference layout to %d nodes (%d hidden)" %
...
func _validate_distribution() -> void:
	...
	if ok:
		print("zuckerwald board: Field distribution VALID (all %d types match Game Bible 3.8)" %
				EXPECTED_DISTRIBUTION.size())
	else:
		push_error("zuckerwald board: Field distribution INVALID — see warnings above")
...
func _get_sorted_nodes() -> Array[NodeBoard]:
	...
	nodes.sort_custom(func(a: NodeBoard, b: NodeBoard) -> bool: return a.name.naturalnocase_to(b.name) < 0)
	return nodes
...
# Game Bible: design/gdd/field-event.md, design/gdd/world-zuckerwald.md
# ============================================================

# Ereignis-Kartei: Definitionen (ID, Name, Gewicht)
const EVENT_DEFINITIONS: Dictionary = 	{
		"klebriger_boden": {
			"name": "Klebriger Boden",
			"weight": 35,
		},
		"zucker_rausch": {
			"name": "Zucker-Rausch",
...
const SPECIAL_RULES: Array[String] = ["floss_fahrt", "zuckerwatte_bonus_minigame"]
...
func _execute_event(event_id: String, player: Node3D, ctrl: Controller) -> void:
	match event_id:
		"klebriger_boden":
			await _ev_klebriger_boden(player, ctrl)
		"zucker_rausch":
			await _ev_zucker_rausch(player, ctrl)
		"keks_regen":
			await _ev_keks_regen(player, ctrl)
		_:
			push_warning("%s: Unknown event \'%s\'" % [board_id, event_id])

# --- Ereignis-Effekte ---

## Klebriger Boden: Aktiver Spieler -2 auf nächsten Wurf (Minimum 1).
