...
# Party Arena: Sternenzitadelle-Board
# Schwierigkeit: ★★★★★
# Game Bible: design/gdd/world-sternenzitadelle.md
...
const REFERENCE_LAYOUT: Array[int] = [
		...
		NodeBoard.FELD_TYP.ITEM_SHOP,  # 7,
		...
		NodeBoard.FELD_TYP.GLUECK_PECH,  # 14,
		...
		NodeBoard.FELD_TYP.GLUECK_PECH,  # 28,
		NodeBoard.FELD_TYP.MUENZ_BONUS,  # 29,
		NodeBoard.FELD_TYP.STERN_SHOP,  # 30,
		...
		NodeBoard.FELD_TYP.STERN_SHOP,  # 35,
		NodeBoard.FELD_TYP.EREIGNIS,  # 36,
		NodeBoard.FELD_TYP.MUENZ_BONUS,  # 37,
...
const EXPECTED_DISTRIBUTION := {
	NodeBoard.FELD_TYP.START: 1,
	NodeBoard.FELD_TYP.STERN_SHOP: 4,
...
# Insel-Metadaten
var board_id: String = "sternenzitadelle"
var board_name: String = "Sternenzitadelle"
var difficulty: int = 5
var item_price_tier: int = 3

# Abzweigungs-Daten (Single Source of Truth: board.json)
# Einstieg/Ausstieg/Δ gemäß Game Bible world-sternenzitadelle.md
const BRANCHES: Array[Dictionary] = [
	{
		"name": "Kosmische Drift",
		"entry": 12,
		"exit": 17,
		"delta": -1,
		"fields": [33, 34, 35, 36],
...
func _apply_reference_layout() -> void:
	if not has_node("Nodes"):
		push_error("sternenzitadelle board: 'Nodes' container not found!")
		return
	...
	if node_boards.is_empty():
		push_error("sternenzitadelle board: No NodeBoard instances found!")
		return
	...
	print("sternenzitadelle board: Applied reference layout to %d nodes (%d hidden)" %
...
func _validate_distribution() -> void:
	...
	if ok:
		print("sternenzitadelle board: Field distribution VALID (all %d types match Game Bible 3.8)" %
				EXPECTED_DISTRIBUTION.size())
	else:
		push_error("sternenzitadelle board: Field distribution INVALID — see warnings above")
...
const SPECIAL_RULES: Array[String] = ["kosmische_drift", "geister_felder"]
...
func _execute_event(event_id: String, player: Node3D, ctrl: Controller) -> void:
	match event_id:
		"sternen_regen":
			await _ev_sternen_regen(player, ctrl)
		"zeit_verzerrung":
			await _ev_zeit_verzerrung(player, ctrl)
		"schwarzes_loch":
			await _ev_schwarzes_loch(player, ctrl)
		"arenastars_segen":
			await _ev_arenastars_segen(player, ctrl)
		_:
			push_warning("%s: Unknown event \'%s\'" % [board_id, event_id])

# --- Ereignis-Effekte ---

## Sternen-Regen: Alle +5 Münzen, aktiver Spieler +3 extra (insgesamt +8).
