...
puppet func _position_update(pos: Vector3, visibility: float):
...
func _server_process(delta: float):
	...
	get_parent().lobby.broadcast_unreliable(self, "_position_update", [self.translation, $Sprite3D.modulate.a])

func knockout(_movement: Vector3):
	get_parent().lobby.broadcast(self, "_client_knockout")
	queue_free()

puppet func _client_knockout():
