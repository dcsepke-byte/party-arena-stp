...
func solo_player(delta: float):
	if not info.is_ai():
		if Input.is_action_just_pressed("player%d_action2" % info.player_id):
			fire()
			get_parent().lobby.broadcast(self, "fire")
	...
	get_parent().lobby.broadcast_unreliable(self, "position_updated", [self.translation, self.rotation, state])
...
puppet func position_updated(translation: Vector3, rotation: Vector3, state: int):
	if self.state == STATE.DEAD or not state in STATE.values() or state == STATE.DEAD:
		return
	if is_solo_player:
...
func _physics_process(delta: float):
	if state == STATE.DEAD:
		...
		if collision != null and multiplayer.is_network_server():
